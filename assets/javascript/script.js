...
async function showEmployees() {
  await fetch(`http://localhost:5001/api/employees/?page=${CurrentPage}&limit=${itemsPerPage}`)
    .then((res) => res.json())
    .then((data) => {
      console.log(data);
  ...
  if (validateForm()) {
    fetch("http://localhost:5001/api/employees", {
      ...
      .then((data) => {
        ...
        // fetch(`http://localhost:5001/api/employees/image`, {
        //   method: "POST",
        //   body: formData,
        // }).then((res) => {
...
async function searchEmployee() {
  ...
  if (searchBar === "" || searchBar === undefined || searchBar === null) {
    ...
    await fetch(`http://localhost:5001/api/employees/search?q=${searchBar}`)
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
...
function close_camcel_del_employee() {
  var delete_btn = document.getElementById("delete_employee");
  delete_btn.style.display = "none";
  var overlay_close = document.getElementsByClassName("overlay")[0];
  overlay_close.style.display = "none";
